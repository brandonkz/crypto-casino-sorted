(function () {
  ...
  const OPERATORS = [
    ...
    { id: 'rainbet', name: 'Rainbet', mark: 'RAIN', color: '#5aa7ff', ink: '#081523', url: '' },
    { id: 'thrill', name: 'Thrill', mark: 'THRL', color: '#ff5c47', ink: '#210909', url: 'https://thrillcasino.io/td4msjnft' },
    { id: 'degen', name: 'Degen', mark: 'DGN', color: '#8ef16c', ink: '#0d1b0b', url: '' },
    { id: 'gamba', name: 'Gamba', mark: 'GMBA', color: '#43d7ff', ink: '#07202a', url: '' },
    { id: 'rollbit', name: 'Rollbit', mark: 'RLBT', color: '#32b8ff', ink: '#071521', url: 'https://rollbit.com/referral/brandonkz' },
    { id: 'betbolt', name: 'BetBolt', mark: 'BOLT', color: '#ebf26d', ink: '#24260a', url: '' },
    { id: 'goated', name: 'Goated', mark: 'GOAT', color: '#ffe28a', ink: '#241804', url: '' },
    { id: 'razed', name: 'Razed', mark: 'RAZD', color: '#ff8f6c', ink: '#2b0d05', url: '' },
    { id: 'c500', name: '500 Casino', mark: '500', color: '#ffe271', ink: '#211905', url: '' },
    ...
    { id: 'metawin', name: 'MetaWin', mark: 'META', color: '#69f0d3', ink: '#06221d', url: '' },
    { id: 'duelbits', name: 'Duelbits', mark: 'DBIT', color: '#a999ff', ink: '#110d28', url: 'https://duelbits.io/?a=sorted' },
    { id: 'shock', name: 'Shock', mark: 'SHCK', color: '#fff27a', ink: '#272205', url: '' },
    { id: 'chips', name: 'Chips.gg', mark: 'CHPS', color: '#f7f7f7', ink: '#151515', url: '' },
    { id: 'bitfortune', name: 'BitFortune', mark: 'BFTN', color: '#ff9f8d', ink: '#240d08', url: '' },
    { id: 'metaspins', name: 'Metaspins', mark: 'MSPN', color: '#cb8bff', ink: '#1b0d24', url: '' },
    { id: 'acebet', name: 'Acebet', mark: 'ACE', color: '#9cd8ff', ink: '#071e2c', url: '' },
    { id: 'spartans', name: 'Spartans', mark: 'SPRT', color: '#ffc16e', ink: '#281602', url: '' },
    { id: 'dicey', name: 'Dicey', mark: 'DICY', color: '#75ffe0', ink: '#06231d', url: '' },
    { id: 'winna', name: 'Winna', mark: 'WINA', color: '#ff91c7', ink: '#260713', url: '' },
    { id: 'onewin', name: '1win', mark: '1WIN', color: '#7fb9ff', ink: '#071523', url: '' },
    { id: 'fortunejack', name: 'FortuneJack', mark: 'FJ', color: '#c3ff8b', ink: '#102406', url: 'https://fortunejack.com' },
    { id: 'degencity', name: 'DegenCity', mark: 'DGCY', color: '#ffa6f4', ink: '#25091d', url: '' },
    { id: 'rolly', name: 'Rolly', mark: 'ROLY', color: '#ffd38f', ink: '#241202', url: '' }
  ];
  ...
  function init() {
    ...
    elements.builderTierList = document.getElementById('builder-tier-list');
    elements.byline = document.getElementById('builder-byline');
    ...
    elements.boardAlert = document.getElementById('builder-board-alert');
  ...
  function renderTopTierRows() {
    const fragment = document.createDocumentFragment();
    const ranking = ensureCompleteRanking(createDefaultRanking());

    for (const tier of TIERS) {
      if (tier.id === 'UNRANKED' && !ranking.UNRANKED.length) continue;
      ...
      if (!ids.length) {
        ...
        ids.forEach((id, index) => zone.appendChild(renderStaticOperatorCard(id, tier.id, index)));
      }
  ...
  function renderBuilderTierRows() {
    const fragment = document.createDocumentFragment();
    ...
    for (const tier of TIERS) {
      ...
      if (!ids.length) {
        ...
        ids.forEach((id, index) => zone.appendChild(renderBuilderOperatorCard(id, tier.id, index)));
      }
    ...
    elements.builderTierList.innerHTML = '';
    elements.builderTierList.appendChild(fragment);
  }

  function renderStaticOperatorCard(operatorId, tierId, index) {
  ...
  function renderBuilderOperatorCard(operatorId, tierId, index) {
    const operator = operatorMap.get(operatorId);
    ...
    if (operator.url) {
      ...
      nameSlot.dataset.tierId = tierId;
  ...
  function onNameClick(event) {
    ...
    const tierId = event.currentTarget.dataset.tierId || findTierForOperator(operatorId);
